...
void Display_Half_Tile(int X_Location, int Y_Location, int TileNumber) {
  for (Ybit = 0;Ybit!=8;Ybit=Ybit+1) {
    for (Xbit = 0;Xbit!=8;Xbit=Xbit+1) {
      if((Tiles[TileNumber].bitmap[Xbit*2][Ybit*2]) > 0) {
       PlotPixel(X_Location+Xbit,Y_Location+Ybit,(Tiles[TileNumber].bitmap[Xbit*2][Ybit*2]));
      } else if ((Tiles[TileNumber].bitmap[Xbit*2+1][Ybit*2]) > 0) {
       PlotPixel(X_Location+Xbit,Y_Location+Ybit,(Tiles[TileNumber].bitmap[Xbit*2+1][Ybit*2]));
      } else if((Tiles[TileNumber].bitmap[Xbit*2][Ybit*2+1]) > 0) {
       PlotPixel(X_Location+Xbit,Y_Location+Ybit,(Tiles[TileNumber].bitmap[Xbit*2][Ybit*2+1]));
      } else if ((Tiles[TileNumber].bitmap[Xbit*2+1][Ybit*2+1]) > 0) {
       PlotPixel(X_Location+Xbit,Y_Location+Ybit,(Tiles[TileNumber].bitmap[Xbit*2+1][Ybit*2+1]));
      } else {
       PlotPixel(X_Location+Xbit,Y_Location+Ybit,0);
     }
   }
  }
}
...
while(1) {
    ...
    Display_One_Tile(144,0,WhichTile);
    Display_One_Tile(128,16,WhichTile);
    Display_Half_Tile(152,24,WhichTile);
  }
}
